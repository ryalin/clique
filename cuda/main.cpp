...
int main(int argc, char *argv[]) {
  ...
  if (checkCorrectness) { // Correctness toggle
    std::cout << "=============== Correctness Test (CUDA) ================" << std::endl;
    Graph test;
    ...
    bool res = parallelCuda(test.nodes, c);
    double simTime = parallelCUDATimer.elapsed();
    ...
    std::cout << "=============== Benchmark Test (CUDA) ================" << std::endl;
    ...
    displayResults(tests, sequentialTimes, parallelCUDATimes);
  }
  return 0;
}